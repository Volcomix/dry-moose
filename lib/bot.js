...
class Bot {
  ...
  async updateFavorites(instruments, bestInstruments) {
    const favorites = await this.retrievefavorites(instruments)
    const obsoletes = this.obsoletes(instruments, bestInstruments, favorites)
    const missings = this.missings(instruments, bestInstruments, favorites)
    await this.removeFavorites(obsoletes)
    await this.addFavorites(missings)
  }

  async retrievefavorites(instruments) {
    const namesToIds = this.namesToIds(instruments)
  ...
  namesToIds(instruments) {
    return Object.values(instruments).reduce(
      (instruments, instrument) => {
        instruments[instrument.symbolFull] = instrument.instrumentId
        return instruments
      }, {}
    )
  }

  obsoletes(instruments, bestInstruments, favorites) {
  ...
  missings(instruments, bestInstruments, favorites) {
